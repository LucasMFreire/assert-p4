...
void TopDeparser() {
  ...
  if(p.ethernet.etherType != 2048){
    printf(":: ETHERNET NOT 2048\n");
  }
  if(a1 && outCtrl.outputPort != 15){
   	printf(":: INCORRECT FORWARD, PACKET SHOULD HAVE DROPPED\n");
  }
  
  if(!required_to_forward1 && outCtrl.outputPort != 15 && outCtrl.outputPort != 14){
   printf(":: set smac action not executed\n");
  }
  
  if(!required_to_forward2 && outCtrl.outputPort != 15 && outCtrl.outputPort != 14){
   printf(":: set dmac action not executed\n");
  }
}
