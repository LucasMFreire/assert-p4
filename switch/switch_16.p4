...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name("parse_gpe_int_header") state parse_gpe_int_header {
        ...
        meta.int_metadata.gpe_int_hdr_len = hdr.vxlan_gpe_int_header.len;
    ...
    @name("parse_int_header") state parse_int_header {
        ...
        meta.int_metadata.instruction_cnt = hdr.int_header.ins_cnt;
...
control process_int_insertion(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name("int_transit") action int_transit(bit<32> switch_id) {
        ...
        meta.int_metadata.gpe_int_hdr_len8 = hdr.int_header.ins_cnt;
    }
    ...
    @name("int_set_header_3") action int_set_header_3() {
        ...
        hdr.int_q_occupancy_header.q_occupancy0 = meta.queueing_metadata.enq_qdepth;
    }
    ...
    @name("int_set_header_2") action int_set_header_2() {
        ...
        hdr.int_hop_latency_header.hop_latency = meta.queueing_metadata.deq_timedelta;
    }
    ...
    @name("int_set_header_0") action int_set_header_0() {
        ...
        hdr.int_switch_id_header.switch_id = meta.int_metadata.switch_id;
    }
    ...
    @name("int_set_header_5") action int_set_header_5() {
        ...
        hdr.int_egress_port_id_header.egress_port_id = standard_metadata.egress_port;
    }
    ...
    @name("int_set_header_4") action int_set_header_4() {
        ...
        hdr.int_ingress_tstamp_header.ingress_tstamp = meta.i2e_metadata.ingress_tstamp;
    }
...
control process_tunnel_encap(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name("inner_non_ip_rewrite") action inner_non_ip_rewrite() {
        meta.egress_metadata.payload_length = standard_metadata.packet_length + (bit<32>)16w65522;
    }
...
control process_ingress_sflow(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name("sflow_ing_pkt_to_cpu") action sflow_ing_pkt_to_cpu(bit<16> sflow_i2e_mirror_id) {
...
control process_mac_acl(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name("acl_mirror") action acl_mirror(bit<16> session_id, bit<14> acl_stats_index, bit<16> acl_meter_index, bit<2> nat_mode, bit<3> ingress_cos, bit<8> tc, bit<2> color) {
        meta.i2e_metadata.mirror_session_id = session_id;
...
control process_system_acl(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name("copp") meter(32w128, CounterType.bytes) copp;
    @name("drop_stats_update") action drop_stats_update() {
        drop_stats_2.count((bit<32>)meta.ingress_metadata.drop_reason);
    }
    @name("nop") action nop() {
    }
    @name("copy_to_cpu") action copy_to_cpu(bit<5> qid, bit<7> meter_id, bit<3> icos) {
        meta.intrinsic_metadata.qid = qid;
        meta.intrinsic_metadata.ingress_cos = icos;
    ...
    @name("redirect_to_cpu") action redirect_to_cpu(bit<5> qid, bit<7> meter_id, bit<3> icos) {
        copy_to_cpu(qid, meter_id, icos);
        mark_to_drop();
        meta.fabric_metadata.dst_device = 8w0;
    }
    @name("copy_to_cpu_with_reason") action copy_to_cpu_with_reason(bit<16> reason_code, bit<5> qid, bit<7> meter_id, bit<3> icos) {
        meta.fabric_metadata.reason_code = reason_code;
        copy_to_cpu(qid, meter_id, icos);
    }
    @name("redirect_to_cpu_with_reason") action redirect_to_cpu_with_reason(bit<16> reason_code, bit<5> qid, bit<7> meter_id, bit<3> icos) {
