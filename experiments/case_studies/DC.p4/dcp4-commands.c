...
int action_run;
int traverse_ingress = 0;
int dst_addr_is_167772161;
int valid_ip_hdr;
...
void end_assertions(){
	if(valid_ip_hdr && traverse_ingress && dst_addr_is_167772161 && assert_forward) {
		printf("Assert error: packet forwarded with ipv4 dstAddr 10.0.0.1\n");
	}
}
...
void parse_arp_rarp() {
	//Extract hdr.arp_rarp
	hdr.arp_rarp.isValid = 1;
	if((hdr.arp_rarp.protoType == 2048)){
		parse_arp_rarp_ipv4();
	} else {
		accept();
	}
...
void parse_cpu_header() {
	...
	if(((hdr.cpu_header.etherType & 63488) == (0 & 63488))){
		parse_snap_header();
	} else if((hdr.cpu_header.etherType == 33024)){
		parse_vlan();
	} else if((hdr.cpu_header.etherType == 37120)){
		parse_vlan();
	} else if((hdr.cpu_header.etherType == 37376)){
		parse_vlan();
	} else if((hdr.cpu_header.etherType == 37632)){
		parse_vlan();
	} else if((hdr.cpu_header.etherType == 2048)){
		parse_ipv4();
	} else if((hdr.cpu_header.etherType == 34525)){
		parse_ipv6();
	} else if((hdr.cpu_header.etherType == 2054)){
		parse_arp_rarp();
	} else if((hdr.cpu_header.etherType == 32821)){
		parse_arp_rarp();
	} else if((hdr.cpu_header.etherType == 35151)){
		parse_nsh();
	} else if((hdr.cpu_header.etherType == 35093)){
		parse_roce();
	} else if((hdr.cpu_header.etherType == 35078)){
		parse_fcoe();
	} else {
		accept();
	}
...
void parse_ethernet() {
	...
	if(((hdr.ethernet.etherType & 63488) == (0 & 63488))){
		parse_snap_header();
	} else if((hdr.ethernet.etherType == 36864)){
		parse_cpu_header();
	} else if((hdr.ethernet.etherType == 268)){
		parse_cpu_header();
	} else if((hdr.ethernet.etherType == 33024)){
		parse_vlan();
	} else if((hdr.ethernet.etherType == 37120)){
		parse_vlan();
	} else if((hdr.ethernet.etherType == 37376)){
		parse_vlan();
	} else if((hdr.ethernet.etherType == 37632)){
		parse_vlan();
		...
		parse_arp_rarp();
	} else if((hdr.ethernet.etherType == 32821)){
		parse_arp_rarp();
	} else if((hdr.ethernet.etherType == 35151)){
		parse_nsh();
	} else if((hdr.ethernet.etherType == 35093)){
		parse_roce();
	} else if((hdr.ethernet.etherType == 35078)){
		parse_fcoe();
	} else {
		accept();
	}
...
void parse_genv_inner() {
	if((hdr.genv.protoType == 25944)){
		parse_inner_ethernet();
	} else if((hdr.genv.protoType == 2048)){
		parse_inner_ipv4();
	} else if((hdr.genv.protoType == 34525)){
		parse_inner_ipv6();
	} else {
		accept();
	}
...
void parse_nsh() {
	...
	if((hdr.nsh.protoType == 2048)){
		parse_inner_ipv4();
	} else if((hdr.nsh.protoType == 34525)){
		parse_inner_ipv6();
	} else if((hdr.nsh.protoType == 25944)){
		parse_inner_ethernet();
	} else {
		accept();
	}
...
void parse_vlan() {
	//Extract hdr.vlan_tag_.next
	hdr.vlan_tag_[hdr.vlan_tag__index].isValid = 1;
	hdr.vlan_tag__index++;
	if((hdr.vlan_tag_[hdr.vlan_tag__index - 1].etherType == 33024)){
		parse_vlan();
	} else if((hdr.vlan_tag_[hdr.vlan_tag__index - 1].etherType == 37120)){
		parse_vlan();
	} else if((hdr.vlan_tag_[hdr.vlan_tag__index - 1].etherType == 37376)){
		parse_vlan();
	} else if((hdr.vlan_tag_[hdr.vlan_tag__index - 1].etherType == 37632)){
		parse_vlan();
	} else if((hdr.vlan_tag_[hdr.vlan_tag__index - 1].etherType == 2048)){
		parse_ipv4();
	} else if((hdr.vlan_tag_[hdr.vlan_tag__index - 1].etherType == 34525)){
		parse_ipv6();
	} else if((hdr.vlan_tag_[hdr.vlan_tag__index - 1].etherType == 2054)){
		parse_arp_rarp();
	} else if((hdr.vlan_tag_[hdr.vlan_tag__index - 1].etherType == 32821)){
		parse_arp_rarp();
	} else {
		accept();
	}
...
void egress() {
	...
	if(action_run == 1203258) {
		 	egress_vlan_xlate_1206257();
...
void cpu_rewrite_1205994() {
	...
	// keys: standard_metadata.egress_port:ternary, standard_metadata.ingress_port:ternary
	// size 512
...
void egress_bd_map_1206069() {
	...
	// keys: meta.ingress_metadata.egress_bd:exact
	// size 512
...
void egress_block_1206128() {
	...
	// keys: standard_metadata.egress_port:exact, meta.intrinsic_metadata.replication_id:exact
	// size 256
...
void egress_system_acl_1206198() {
	...
	// keys: meta.egress_metadata.mtu_check_fail:ternary
	// size 512
...
void egress_vlan_xlate_1206257() {
	...
	// keys: standard_metadata.egress_port:exact, meta.egress_metadata.bd:exact
	// size 512
...
void outer_mac_1206333() {
	...
	// keys: meta.egress_metadata.smac_idx:exact, hdr.ipv4.dstAddr:ternary
	// size 512
...
void rewrite_1206410() {
	...
	// keys: meta.ingress_metadata.nexthop_index:exact
	// size 512
...
void rid_1206499() {
	...
	// keys: meta.intrinsic_metadata.replication_id:exact
	// size 512
...
void tunnel_decap_1206558() {
	...
	// keys: meta.ingress_metadata.tunnel_type:exact, hdr.inner_ipv4.$valid$:exact, hdr.inner_tcp.$valid$:exact, hdr.inner_udp.$valid$:exact
	// size 512
...
void tunnel_dst_rewrite_1206677() {
	...
	// keys: meta.egress_metadata.tunnel_dst_index:exact
	// size 512
...
void tunnel_rewrite_1206730() {
	...
	// keys: meta.egress_metadata.tunnel_type:exact, hdr.ipv4.$valid$:exact, hdr.tcp.$valid$:exact, hdr.udp.$valid$:exact
	// size 256
...
void tunnel_src_rewrite_1206861() {
	...
	// keys: meta.egress_metadata.tunnel_src_index:exact
	// size 512
...
void ingress() {
	if((meta.ingress_metadata.ingress_bypass == 0)) {
		traverse_ingress = 1;
		valid_ip_hdr = hdr.ipv4.isValid;
		dst_addr_is_167772161 = (hdr.ipv4.dstAddr == 167772161);
...
void bd_1_1208666() {
	...
	// keys: meta.ingress_metadata.bd:exact
	// size 512
...
void dmac_1_1208719() {
	...
	// keys: meta.ingress_metadata.bd:exact, meta.ingress_metadata.lkp_mac_da:exact
	// size 512
...
void ecmp_group_1208817() {
	...
	// keys: meta.ingress_metadata.ecmp_index:exact, meta.ingress_metadata.lkp_ipv4_sa:selector, meta.ingress_metadata.lkp_ipv4_da:selector, meta.ingress_metadata.lkp_ip_proto:selector, meta.ingress_metadata.lkp_l4_sport:selector, meta.ingress_metadata.lkp_l4_dport:selector
	// size 512
...
void fwd_result_1208959() {
	...
	// keys: meta.ingress_metadata.l2_redirect:ternary, meta.ingress_metadata.acl_redirect:ternary, meta.ingress_metadata.racl_redirect:ternary, meta.ingress_metadata.fib_hit:ternary
	// size 512
...
//Table
void ip_acl_1209208() {
	if(meta.ingress_metadata.lkp_ipv4_da == 167772161){
		racl_deny_0_1207635();
	}
}
...
void ip_racl_1209438() {
	...
	// keys: meta.ingress_metadata.bd_label:ternary, meta.ingress_metadata.lkp_ipv4_sa:ternary, meta.ingress_metadata.lkp_ipv4_da:ternary, meta.ingress_metadata.lkp_ip_proto:ternary, meta.ingress_metadata.lkp_l4_sport:ternary, meta.ingress_metadata.lkp_l4_dport:ternary
	// size 512
...
void ipv4_dest_vtep_1209581() {
	...
	// keys: meta.ingress_metadata.vrf:exact, meta.ingress_metadata.lkp_ipv4_da:exact, meta.ingress_metadata.lkp_ip_proto:exact, meta.ingress_metadata.lkp_l4_dport:exact
	// size 512
...
void ipv4_fib_1209676() {
	...
	// keys: meta.ingress_metadata.vrf:exact, meta.ingress_metadata.lkp_ipv4_da:exact
	// size 512
...
void ipv4_fib_lpm_1209753() {
	...
	// keys: meta.ingress_metadata.vrf:exact, meta.ingress_metadata.lkp_ipv4_da:lpm
	// size 512
...
void ipv4_src_vtep_1209824() {
	...
	// keys: meta.ingress_metadata.vrf:exact, meta.ingress_metadata.lkp_ipv4_sa:exact
	// size 512
...
void lag_group_1209901() {
	...
	// keys: meta.ingress_metadata.egress_ifindex:exact, meta.ingress_metadata.lkp_mac_sa:selector, meta.ingress_metadata.lkp_mac_da:selector, meta.ingress_metadata.lkp_mac_type:selector, meta.ingress_metadata.lkp_ipv4_sa:selector, meta.ingress_metadata.lkp_ipv4_da:selector, meta.ingress_metadata.lkp_ip_proto:selector, meta.ingress_metadata.lkp_l4_sport:selector, meta.ingress_metadata.lkp_l4_dport:selector
	// size 512
...
void learn_notify_1210073() {
	...
	// keys: meta.ingress_metadata.l2_src_miss:ternary, meta.ingress_metadata.l2_src_move:ternary, meta.ingress_metadata.stp_state:ternary
	// size 512
...
void mac_acl_1210156() {
	...
	// keys: meta.ingress_metadata.if_label:ternary, meta.ingress_metadata.bd_label:ternary, meta.ingress_metadata.lkp_mac_sa:ternary, meta.ingress_metadata.lkp_mac_da:ternary, meta.ingress_metadata.lkp_mac_type:ternary
	// size 512
...
void mirror_acl_1210275() {
	...
	// keys: meta.ingress_metadata.if_label:ternary, meta.ingress_metadata.bd_label:ternary, meta.ingress_metadata.lkp_ipv4_sa:ternary, meta.ingress_metadata.lkp_ipv4_da:ternary, meta.ingress_metadata.lkp_ip_proto:ternary, meta.ingress_metadata.lkp_mac_sa:ternary, meta.ingress_metadata.lkp_mac_da:ternary, meta.ingress_metadata.lkp_mac_type:ternary
	// size 512
...
void outer_rmac_1210483() {
	...
	// keys: meta.ingress_metadata.outer_rmac_group:exact, meta.ingress_metadata.lkp_mac_da:exact
	// size 256
...
void rmac_1210731() {
	...
	// keys: meta.ingress_metadata.rmac_group:exact, meta.ingress_metadata.lkp_mac_da:exact
	// size 512
...
void smac_1_1210802() {
	...
	// keys: meta.ingress_metadata.bd:exact, meta.ingress_metadata.lkp_mac_sa:exact
	// size 512
...
void spanning_tree_1210879() {
	...
	// keys: meta.ingress_metadata.ifindex:exact, meta.ingress_metadata.stp_group:exact
	// size 512
...
void system_acl_1210944() {
	...
	// keys: meta.ingress_metadata.if_label:ternary, meta.ingress_metadata.bd_label:ternary, meta.ingress_metadata.lkp_ipv4_sa:ternary, meta.ingress_metadata.lkp_ipv4_da:ternary, meta.ingress_metadata.lkp_ip_proto:ternary, meta.ingress_metadata.lkp_mac_sa:ternary, meta.ingress_metadata.lkp_mac_da:ternary, meta.ingress_metadata.lkp_mac_type:ternary, meta.ingress_metadata.acl_deny:ternary, meta.ingress_metadata.racl_deny:ternary, meta.ingress_metadata.src_vtep_miss:ternary, meta.ingress_metadata.routed:ternary, meta.ingress_metadata.src_is_link_local:ternary, meta.ingress_metadata.ttl:ternary, meta.ingress_metadata.stp_state:ternary, meta.ingress_metadata.control_frame:ternary, standard_metadata.egress_spec:ternary
	// size 256
...
void tunnel_1211206() {
	...
	// keys: meta.ingress_metadata.tunnel_vni:exact, meta.ingress_metadata.tunnel_type:exact, hdr.inner_ipv4.$valid$:exact
	// size 512
...
void validate_outer_ethernet_1211283() {
	...
	// keys: hdr.ethernet.dstAddr:ternary
	// size 64
...
void validate_outer_ipv4_packet_1211348() {
	...
	// keys: hdr.ipv4.version:exact, hdr.ipv4.ihl:exact, hdr.ipv4.ttl:exact, hdr.ipv4.srcAddr:ternary, hdr.ipv4.dstAddr:ternary
	// size 64
...
void validate_packet_1211455() {
	...
	// keys: meta.ingress_metadata.lkp_mac_da:ternary, meta.ingress_metadata.lkp_ipv4_da:ternary
	// size 64
